                                    ...
                                    {editingId === unit.id ? (
                                        ...
                                        <div className="flex-1">
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                {unit.proofUrl && (
                                                    <a href={unit.proofUrl} target="_blank" className="text-xs font-bold text-primary underline ml-2">View Proof</a>
                                                )}
                                            </div>
                                    ...
                                    <div className="flex items-center gap-2 w-full md:w-auto mt-4 md:mt-0">
                                        {editingId === unit.id ? (
                                            <>
                                                ...
                                                <button onClick={() => handleApprove(unit.id, editForm)} className="flex-1 md:flex-none bg-success text-white px-4 py-2 rounded-lg font-bold text-sm shadow-sm hover:bg-green-600 transition-colors">
                                                    Confirm
                                                </button>
                                            </>
                                        ) : (
                                            <>
                                                <button onClick={() => {
                                                    setEditForm({ cpu: unit.cpu || '', gpu: unit.gpu || '', ram: unit.ram || '', storage: unit.storage || '' });
                                                    setEditingId(unit.id);
                                                }} className="p-2 text-slate-400 hover:text-primary transition-colors hover:bg-primary/10 rounded-lg">
                                                    <Edit3 size={20} />
                                                </button>
                                                <button onClick={() => handleApprove(unit.id, { cpu: unit.cpu, gpu: unit.gpu, ram: unit.ram, storage: unit.storage })} className="flex-1 md:flex-none bg-carbon text-white px-4 py-2 rounded-lg font-bold text-sm shadow-sm hover:bg-black transition-colors">
                                                    Approve
                                                </button>
                                            </>
                                        )}
