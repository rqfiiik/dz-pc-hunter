...
export default function Register() {
    ...
    const { login } = useAuth(); // Import login so we can auto-login via OAuth
    ...
    const googleLoginFlow = useGoogleLogin({
        onSuccess: async (tokenResponse) => {
            ...
            try {
                ...
                console.error("Google registry error:", err);
    ...
    const handleFacebookResponse = async (response) => {
        if (response.accessToken) {
            ...
            try {
                ...
                console.error("Facebook registry error:", err);
        ...
        <main className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
            <div className="bg-white max-w-md w-full rounded-2xl shadow-sm border border-gray-100 p-8">
                <div className="text-center mb-8">
                    <h1 className="text-2xl font-bold text-carbon">Create an Account</h1>
                    <p className="text-slate-500 mt-2">Unlock unlimited PC market intelligence.</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full bg-primary text-white font-bold py-3 rounded-lg mt-4 hover:bg-primary-hover transition-colors rounded-xl disabled:opacity-50"
                    >
                ...
                <p className="text-center text-sm text-slate-500 mt-8">
                    Already have an account? <Link href="/login" className="text-carbon font-bold hover:underline">Sign In</Link>
                </p>
            </div>
        </main>
    );
